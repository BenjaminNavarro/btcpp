...
BTGui::BTGui(NodeStyle style) : style_{style} {
}

void BTGui::set_node_style(NodeStyle style) {
    style_ = style;
    draw_graph();
}

QGraphicsScene* BTGui::scene() {
    return &scene_;
}

void BTGui::set_graph(btcpp::GraphData data) {
    data_ = std::move(data);
    node_widgets_.clear();
...
void BTGui::update_graph_state(const btcpp::GraphData& data) {
    if (data_.size() != data.size()) {
        std::cout << "Can't update graph: node count mismatch\n";
        return;
    }
...
void BTGui::draw_graph() {
    layout_ = compute_graph_layout(data_, style_);
    draw_edges();
    draw_nodes();
}

void BTGui::draw_edges() {
    for (int id = 0; const auto& node_data : data_) {
        ...
        for (const int child_id : node_data.children) {
            const auto start =
                parent_position +
                QPointF{style_.width / 2.0, static_cast<double>(style_.height)};
            const auto child_position = to_qpointf(layout_.at(child_id));
            const auto end = child_position + QPointF{style_.width / 2.0, 0.0};
...
QString BTGui::node_type_text(int id, const btcpp::NodeData& node_data) {
...
QPointF BTGui::to_qpointf(const auto& pos) {
    return {static_cast<double>(pos.x), static_cast<double>(pos.y)};
};

QPoint BTGui::to_qpoint(const auto& pos) {
